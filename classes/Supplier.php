...
class SupplierCore extends ObjectModel
{
        ...
        $order_by = null, $order_way = null, $get_total = false, $active = true, $active_category = true)
    {
        ...
        if (Group::isFeatureActive()) {
            $groups = FrontController::getCurrentCustomerGroups();
            empty($groups) && ($groups = array((int)Configuration::get('PS_UNIDENTIFIED_GROUP')));
            $sql_groups = 'WHERE cg.`id_group` IN ('.implode(',', $groups).')';
        }
        ...
        if (Group::isFeatureActive() || $active_category) {
            ...
            if (Group::isFeatureActive()) {
                $sql .= 'JOIN `'._DB_PREFIX_.'category_group` cg ON (cp.`id_category` = cg.`id_category` AND cg.`id_group` '.(count($groups) ? 'IN ('.implode(',', $groups).')' : '= '.(int)Configuration::get('PS_UNIDENTIFIED_GROUP')).')';
            }
            if ($active_category) {
